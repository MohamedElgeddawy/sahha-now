...
const Announcements = ({ className = "" }: AnnouncementsProps) => {
  ...
  // Responsive: Carousel with dots for mobile/tablet, grid for desktop
  if (isMobile || isTablet) {
    const itemsToShow = isTablet ? 2 : 1;
    const itemWidth = 100 / itemsToShow;
    const totalSlides = Math.ceil(announcements.length / itemsToShow);
      ...
      <div className={`w-full mx-auto px-4 ${className}`}>
        <div className="relative w-full">
          ...
          <div className="overflow-hidden relative w-full">
            ...
            >
              {Array(totalSlides).fill(null).map((_, slideIndex) => (
                <div 
                  key={slideIndex}
                ...
                >
                  {announcements
                    ...
                    .map((announcement) => (
                      <div
                        key={announcement.id}
                        className={`flex-shrink-0 ${isTablet ? 'w-1/2 px-2' : 'w-full px-1'}`}
                      >
                        {/* Announcement Card */}
                        <div className="relative h-[320px] rounded-xl overflow-hidden shadow-lg group bg-gradient-to-r from-blue-50 to-green-50 mx-auto"
                          style={{ maxWidth: isTablet ? 'calc(100% - 8px)' : '100%' }}>
                          {/* Image */}
                          <div
                            className="absolute rounded-lg overflow-hidden shadow-md"
                            style={{
                          ...
                          {/* Content */}
                          <div className="absolute right-4 left-[210px] top-12 flex flex-col gap-2 max-w-[calc(100%-210px-1rem)]">
                            <motion.h2
                              initial={{ opacity: 0, x: 20 }}
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ duration: 0.5, delay: 0.2 }}
                              className="text-base xs:text-lg sm:text-xl font-bold mb-1 leading-tight text-gray-800"
                            >
                              {announcement.title}
                            </motion.h2>
                            <motion.p
                              ...
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ duration: 0.5, delay: 0.3 }}
                              className="text-xs xs:text-sm sm:text-base mb-2 line-clamp-3 text-gray-400"
                            >
                              {announcement.description}
                            </motion.p>
                            <motion.div
                            ...
                            >
                              <Button
                                asChild
                                className="w-full sm:w-auto px-4 py-2 text-xs sm:text-sm font-semibold rounded-lg shadow-md bg-green-600 text-white hover:bg-green-700"
                              >
                                <Link href={announcement.redirectUrl}>
                                  {announcement.buttonText || "اشترى الآن"}
                                </Link>
